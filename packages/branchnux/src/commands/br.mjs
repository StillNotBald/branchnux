 ...
 * src/commands/br.mjs
 *
 * Implements `branchnux br` — Business Requirements (BR-XX) management.
 *
 * Sub-commands:
...
export async function runBrInit(id, opts = {}) {
  ...
  if (!fs.existsSync(filePath)) {
    ...
    const header = [
      ...
      '',
      '<!-- branchnux: BUSINESS_REQUIREMENTS v1 -->',
      '<!-- Each ## BR-XX section is managed by `branchnux br`. -->',
      '',
    ].join('\n');
  ...
  if (!json) {
    ...
    console.log(`  branchnux br link ${id} R-01,R-02`);
  }
}
...
export async function runBrLink(brId, rIds, opts = {}) {
  ...
  if (!fs.existsSync(filePath)) {
    const err = new Error(
      `${filePath} not found. Run \`branchnux br init ${brId}\` first.`
    );
    err.exitCode = 2;
    throw err;
  }
  ...
  if (!hasBrSection(content, brId)) {
    const err = new Error(
      `${brId} section not found in ${filePath}. Run \`branchnux br init ${brId}\` first.`
    );
    err.exitCode = 2;
    throw err;
  }
...
export async function runBrRtm(opts = {}) {
  ...
  if (!fs.existsSync(brFile)) {
    if (!json) {
      ...
      console.log('  Run: branchnux br init BR-01');
    }
  ...
  const rtmContent = [
    ...
    '',
    '<!-- branchnux: UAT_TRACEABILITY v2 — generated by `branchnux br rtm` -->',
    '<!-- Do not hand-edit the table; re-run `branchnux br rtm` to refresh. -->',
    '',
 ...
 * Used by `branchnux sign` to determine if attestation completes the BR.
 *
 * @param {string} brId             - e.g. "BR-01"
...
function buildBrSection(id) {
  return [
    ...
    '### Linked R-IDs',
    '',
    '_(none yet — run `branchnux br link ' + id + ' R-01,R-02`)_',
    '',
    '### Linked TC-IDs',
    '',
