 ...
 *
 * Implements `branchnux sign stale-check <surface>` — flag sign-off entries
 * that have exceeded the configured age threshold.
 *
 * Usage:
 *   branchnux sign stale-check <surface> [--threshold 90d] [--strict]
...
export async function runSignStaleCheck(surface, opts = {}) {
  ...
  if (!json) {
    ...
    if (staleEntries.length === 0) {
      ...
      for (const e of staleEntries) {
        console.log(`    branchnux sign ${surface} --tc ${e.tc_id}`);
      }
    }
